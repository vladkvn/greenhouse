...
class SimExplorer:
    ...
    def __init__(
        ...
        *,
        ...
        self._follower = ReactiveLocalPlanner(
    ...
    def update(self) -> bool:
        ...
        if self._last_xy is not None and pose.point.distance_to(self._last_xy) < 0.01:
            self._stuck += 1
        else:
            self._stuck = 0
        self._last_xy = pose.point
        if self._stuck > self._stuck_limit and self._goal is not None:
            self._blacklist.add(built.meta.world_to_cell(self._goal.point))
            self._goal, self._stuck = None, 0
