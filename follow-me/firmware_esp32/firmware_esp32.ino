 ...
 *
 * !!! БЕЗОПАСНОСТЬ ПРИ ВКЛЮЧЕНИИ (чтобы моторы не дёргались на старте) !!!
 *   1. EN-пины (ENA/ENB) — только на "тихие" при загрузке GPIO (здесь 13 и 32).
 *      Пины 0,2,5,12,14,15 при boot шумят и крутанут мотор до init — не брать их под EN.
 *   2. Снять джамперы ENA/ENB на плате L298 (иначе EN всегда +5В = мотор не выключить).
 *   3. Поставить pulldown 10 кОм с ENA и ENB на GND — держит мост выключенным,
 *      пока ESP32 грузится (~300 мс пины floating до setup()).
 *   4. Питание ESP32 и моторов — РАЗДЕЛЬНОЕ (общий только GND). Иначе пусковой ток
 *      моторов роняет 3.3В, ESP32 ребутится по кругу и моторы дёргаются.
 *   5. Первое включение — с ОТКЛЮЧЁННЫМ питанием моторов: убедиться, что после boot
 *      всё тихо, и только потом подавать питание на L298.
 */
...
// ВАЖНО: EN-пины НЕ должны "шуметь" при загрузке, иначе моторы дёрнутся до
// инициализации. Поэтому EN — на GPIO, которые при boot тихие и LOW (13, 32).
// НЕ использовать 0,2,5,12,14,15 для EN — они выдают PWM/высокий уровень при старте.
// EN-пины желательно ещё притянуть к GND резистором 10 кОм (см. шапку файла).
...
const int PIN_L_EN  = 13;   // L298 ENA  (ШИМ левого борта) — снять джампер ENA, +pulldown 10k!
const int PIN_R_EN  = 32;   // L298 ENB  (ШИМ правого борта) — снять джампер ENB, +pulldown 10k!
// ======================================================================
